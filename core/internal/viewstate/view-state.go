...
// switching back to the normal mode when the view-change opreation is
// complete.
package viewstate
...
func (s *viewState) WaitAndHoldActiveView() (view uint64, release func()) {
	...
	for s.lastActiveView != s.lastNewView {
...
func (s *viewState) WaitAndHoldView(view uint64) (ok bool, release func()) {
	...
	for view != s.lastActiveView || s.lastActiveView != s.lastNewView {
		if view < s.lastNewView {
			return false, nil
		}
...
func (s *viewState) RequestViewChange(newView uint64) bool {
	...
	return newView > s.lastActiveView
}
...
func (s *viewState) FinishViewChange(newView uint64) (ok bool, release func()) {
	...
	for {
		if newView < s.lastNewView || newView <= s.lastActiveView {
			return false, nil
		}
