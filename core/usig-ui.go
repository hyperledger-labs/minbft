...
// UIs generated by that replica have been accepted and committed. If
// the UI cannot be accepted immediately, it will block until the UI
// can be accepted. The return value new indicates if the UI has not
// been accepted before. It is safe to invoke concurrently.
type uiAcceptor func(msg messages.MessageWithUI) (new bool, err error)

// uiCommitter records a unique identifier of a message as committed.
//
...
type uiCommitter func(msg messages.MessageWithUI)
...
// makeUIAcceptor constructs uiAcceptor using the supplied interfaces.
func makeUIAcceptor(providePeerState peerstate.Provider, verifyUI uiVerifier) uiAcceptor {
	return func(msg messages.MessageWithUI) (new bool, err error) {
		...
		return peerState.AcceptUI(ui), nil
	}
}

// makeUICommitter constructs uiCommitter using the supplied peer
...
func makeUICommitter(providePeerState peerstate.Provider) uiCommitter {
	return func(msg messages.MessageWithUI) {
		...
		if err := peerState.CommitUI(ui); err != nil {
			panic(err)
		}
	}
}
